...
for i in range(len(NUM_MI)):
    assert (
        (NUM_MI[i] == len(RANGE_NAMES[i]))
        & (NUM_MI[i] == len(RANGE_BASE_ADDR[i]))
        & (NUM_MI[i] == len(RANGE_ADDR_WIDTH[i]))
    ), (
        "Mismatch in lenght of configurations: NUM_MI("
        + str(NUM_MI[i])
        + "), RANGE_NAMES ("
        + str(len(RANGE_NAMES[i]))
        + "), RANGE_BASE_ADDR("
        + str(len(RANGE_BASE_ADDR[i]))
        + ") RANGE_ADDR_WIDTH("
        + str(len(RANGE_ADDR_WIDTH[i]))
        + ")"
    )
